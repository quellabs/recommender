...
HELP;
		...
		public function execute(ConfigurationManager $config): int {
			...
			if ($categoryArg !== null) {
				...
				$rows = $connection->execute(
					'SELECT DISTINCT category FROM vogoo_ratings ORDER BY category'
				)->fetchAll('assoc');
		 ...
		 */
		private function rebuildCategory(\Cake\Database\Connection $connection, RecommendationConfig $config, int $category): void {
			...
			$connection->execute(
				'DELETE FROM vogoo_links WHERE category = :category',
				['category' => $category],
			);
		...
		private function rebuildMemberPairs(\Cake\Database\Connection $connection, RecommendationConfig $config, int $memberId, int $category, float $threshold): void {
				...
				$ratings = $connection->execute(
					...
					['member_id' => $memberId, 'category' => $category],
				)->fetchAll('assoc');
					...
					for ($j = $i + 1; $j < $count; $j++) {
						...
						if ($config->isDirectLinks()) {
							if ($ratingA >= $threshold && $ratingB >= $threshold) {
								$this->upsertLink($connection, (int)$a['product_id'], (int)$b['product_id'], $category, 1, 0.0);
								$this->upsertLink($connection, (int)$b['product_id'], (int)$a['product_id'], $category, 1, 0.0);
							}
						}
		 ...
		 */
		private function upsertLink(\Cake\Database\Connection $connection, int $itemId1, int $itemId2, int $category, int $cntDelta, float $diffDelta): void {
			$connection->execute(
